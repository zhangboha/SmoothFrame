...
//        }
    ...
    public void setSwipeRefshDistance(final SwipeRefreshLayout swipeLayout,final Context context,final int distance){
        ViewTreeObserver vto = swipeLayout.getViewTreeObserver();
        vto.addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            public void onGlobalLayout() {

                final DisplayMetrics metrics = context.getResources().getDisplayMetrics();
                Float mDistanceToTriggerSync = Math.min(((View) swipeLayout.getParent()).getHeight() * 0.6f, distance * metrics.density);
